...
from core.attack.attack_by_sharpness import attack_label, attack_label_fixed_steps_time, AGD_cmp

import torch
from torch.utils.data import DataLoader, TensorDataset
from pprint import pformat

from embedding_dict import extract_embeddings
from core.AGD_Detector import AGDDetector
...
def main():
    ...
    x_dataset, y_dataset = extract_embeddings(model, x_train, y_train)
    # 如果 extract_embeddings 已经返回 Tensor，就只做 .to(device)
    
    x_dataset = x_dataset.to(device, non_blocking=True)
    y_dataset = y_dataset.to(device, non_blocking=True)

    logging.debug(green('Test the target model...'))

    x_train_tensor = torch.tensor(x_train).float()
    x_test_tensor = torch.tensor(x_test).float()
    x_adv_tensor = torch.tensor(x_adv).float()
    ...
    detection_classifier_attacker_dir = os.path.join(config['GradPrint_result_dir'], "_".join(
    ...
    if args.performance_fixed_param:
        ...
        _, test_criteria_all = \
            AGD_cmp(model, test_loader, x_train_tensor, x_dataset, y_dataset, epsilon, device)

        # perturb on adv sample
        _, adv_criteria_all = \
            AGD_cmp(model, adv_loader, x_train_tensor, x_dataset, y_dataset, epsilon, device)
    ...
    if args.time_consume:
        ...
        _, test_criteria_all = \
            AGD_cmp(model,  dataset.time_test_loader, x_train_tensor, x_dataset, y_dataset, epsilon, device)
    ...
    #     # perturb on test sample
    #     _, test_criteria_all = \
    #         attack_label_fixed_steps_time(model, test_loader, x_dataset, init_mag, lr, perturbation_steps, detect_type, device)

    #     # perturb on adv sample
    #     _, adv_criteria_all = \
    #         attack_label_fixed_steps_time(model, adv_loader, x_dataset, init_mag, lr, perturbation_steps, detect_type, device)
    ...
    #     save_fir = os.path.join(config['GradPrint_result_dir'], 'all_classifier2', file_name)
...
def parse_args():
    ...
    p.add_argument('--enhance_performance', action='store_true', help='enhance_performance with/without GradPrint false positive & false negative')
