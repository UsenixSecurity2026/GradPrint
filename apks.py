...
class ApkData:
    ...
    def __init__(self, detection, granularity, classifier, attacker, base_clf_dir=config['base_clf_dir'], data_source=config['data_source'], dim_to_be_changed = True, time_test_size = 10, args=None):
        ...
        if dim_to_be_changed:
            ...
            self.time_test_loader = DataLoader(TensorDataset(torch.tensor(self.x_train[:time_test_size]).float(), torch.tensor(self.y_train[:time_test_size]).float()), shuffle=True, batch_size=1)
        else:
            # train_loader、test_loader
            if detection == "drebin" or detection == "apigraph":
                ...
                self.test_loader = DataLoader(TensorDataset(torch.tensor(self.x_test.toarray()).float(), torch.tensor(self.y_test).float()), shuffle=False, batch_size=10)
    ...
    def load_data(self, detection, granularity, classifier, attacker, data_source, args, isOneHotEncoding = True):
        ...
        if detection == "drebin" or detection == "apigraph":
            ...
            x_train = np.load(os.path.join(data_source, attacker, 'train_sample',
                                           detection_granularity_dir, detection + '_' + classifier + '_train.npy'))
            x_test = np.load(os.path.join(data_source, attacker, 'test_sample',
        ...
        y_test = np.load(os.path.join(data_source, attacker, 'test_sample',
                                      detection_granularity_dir, detection + '_' + classifier + '_test_label.npy'))
        y_train = np.load(os.path.join(data_source, attacker, 'train_sample',
        ...
        if attacker == 'AdvDroidZero':
            ...
            base_clf_predict_y_train = np.load(os.path.join(data_source, attacker, 'train_sample',
